describe "Nested Structures" do
  ...
  it 'adds a student to the school student array' do 
    school[:students].push({:name =>'Dee', :grade => 'A'})
  ...
  it "changes Frank's grade from 'A' to 'F'" do 
      #reset schools data because other tests are changing it
    school = {
      :name => 'Happy Funtime School',
      :location => 'NYC',
      :instructors => [
  ...
  it "returns the name of the student with a 'B'" do
    expect( school[:students].select{ |student| student[:grade] == "B" }[0][:name] ).to eq("Marissa")
  end

  it "returns the subject of the instructor 'Jeff'" do 
    expect(school[:instructors].select{ |instructor| instructor[:name] == "Jeff" }[0][:subject] ).to eq("karaoke")
  end
end